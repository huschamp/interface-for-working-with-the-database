...
class DB_GUI(QtWidgets.QMainWindow):
    ...
    def change_cortege(self):
        ...
        #self.dlgc_ui.tableWidget.setColumnCount(len(self.table_header))
        #self.dlgc_ui.tableWidget.setHorizontalHeaderLabels(self.table_header)
        self.dlgc.show()
    ...
    def selectw(self):
        ...
        try:
            ...
            if attr:
           #     insert = sql.SQL(self.sql_command["selwhe"].format(attr,self.table_name,usl))
           #     self.cursor.execute(insert)	
           #     self.get_table()
                self.sel_table()
                #self.attribute_list()
       ...
       # s=self.selectw.attr
        ...
        if self.table_name:
          ...
          #  self.table_header = []
           ...
           ### self.cursor.execute(self.sql_command["select"].format(s, self.table_name))
            ...
            while i < l:
                ...
                if k==1:
                    for j in self.cursor:
                       # if self.table_header[j] == s:
                            #print(self.table_header[j])
                           # print(table_header)
                            self.table.append(j)
                else:
    ...
    def save_as(self):
        self.fname = QFileDialog.getSaveFileName(self, 'Save as...', '/BD.csv')[0]
        try:
            with open(self.fname, "w", errors='ignore', encoding='utf-8', newline='') as f:
                writer = csv.writer(f, delimiter=';')
                writer.writerow(self.table_header)
